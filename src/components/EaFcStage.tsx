...
export function EaFcStage({ active, isWalkout = false, children }: Props) {
  ...
  useEffect(() => {
    ...
    // Trigger flash burst if newly revealed walkout
    if (isWalkout) {
      setFlash(true);
  ...
  return (
    <div className="eafc-stage-container relative flex h-full w-full items-center justify-center overflow-hidden">
      {/* 1. Flash Burst Inicial do Walkout */}
      {flash && (
        <div className="eafc-flash-burst pointer-events-none fixed inset-0 z-50 bg-radial from-white via-gold/40 to-transparent" />
      )}
      ...
      <div className="pointer-events-none absolute inset-0 z-0 overflow-hidden">
        ...
        <div className="eafc-pedestal absolute bottom-0 left-1/2 h-44 w-[120%] -translate-x-1/2" />

        {/* Partículas / Embers Dourados Flutuantes */}
        {embers.map((e) => (
          <div
            key={e.id}
            className="eafc-ember absolute bottom-6 rounded-full bg-[#FFE27A] shadow-[0_0_8px_#D4AF37]"
            style={{
              left: `${e.left}%`,
              width: `${e.size}px`,
              height: `${e.size}px`,
      ...
      <div className={`relative z-10 flex items-center justify-center ${isWalkout ? "eafc-walkout-entrance" : "eafc-card-float"}`}>
        {children}
      </div>
    </div>
  );
